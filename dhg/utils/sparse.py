...
def dense_dropout(mat:torch.Tensor, p: float, fill_value: float = 0.0) -> torch.Tensor:
    r"""Dropout function for dense matrix. This function will return a new dense matrix with the
    same shape as the input dense matrix, but with some elements dropped out.
    """
    assert 0 <= p <= 1
    if p == 0:
        return mat
    p = torch.ones(mat.shape[0], device=mat.device) * p
    keep_mask = torch.bernoulli(1 - p).to(mat.device)
    fill_values = torch.logical_not(keep_mask) * fill_value
    new_mat = mat * keep_mask + fill_values
    return new_mat