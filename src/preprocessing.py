...
#     First, ensure the sorting is correct (increasing along edge_index[0])
    ...
    if not ((data.n_x+data.num_hyperedges-1) == data.edge_index[0].max().item()):
        raise ValueError('num_hyperedges does not match! ')
        return
...
def rand_train_test_idx(label, train_prop=.5, valid_prop=.25, ignore_negative=True, balance=False):
    ...
    if not balance:
        ...
        train_mask = index_to_mask(train_indices, label.size(0))
        val_mask = index_to_mask(val_indices, label.size(0))
        test_mask = index_to_mask(test_indices, label.size(0))
...
def kfold_train_test_idx(label, k=5, train_prop=.5):
    """ Adapted from https://github.com/CUAI/Non-Homophily-Benchmarks"""
    """ randomly splits label into train/valid/test splits """
    ...
    for _ in range((k-1)):
        ...
        temp_perm = torch.cat([perm[0:test_start], perm[test_end:]])
        train_indices = temp_perm[0:train_num]
        val_indices = temp_perm[train_num:]
        ...
        test_start = test_end
    test_indices = perm[test_start:]
    temp_perm = perm[0:test_start]
    train_indices = temp_perm[0:train_num]
    val_indices = temp_perm[train_num:]
...
def incidence_to_edge_index(H: torch.Tensor) -> torch.LongTensor:
    """
    Convert an n x m incidence matrix H into a PyG-compatible edge_index tensor.
    Each 1 in H[i, j] means node i is in hyperedge j.

    The returned edge_index is a 2 x (2 * num_edges) LongTensor
    where edges are bidirectional: node <-> hyperedge.

    Args:
        H (torch.Tensor): incidence matrix of shape (n_nodes, n_hyperedges)

    Returns:
        torch.LongTensor: edge_index of shape (2, 2 * num_edges)
    """
    n_nodes, n_hyperedges = H.shape

    # Find the (i,j) indices where H[i,j] == 1
    node_indices, he_indices = torch.nonzero(H, as_tuple=True)

    # Offset hyperedge indices to avoid collision with node indices
    he_indices_offset = he_indices + n_nodes

    # One direction: node -> hyperedge
    row1 = torch.cat([node_indices, he_indices_offset])
    row2 = torch.cat([he_indices_offset, node_indices])
...
# def incidence_to_edge_index2(H: torch.Tensor) -> torch.Tensor:
#     """
#     Converts an incidence matrix H (n x m) to a PyTorch Geometric edge_index
#     in bipartite format: edge_index = [V -> E; E -> V], both directions.

#     Args:
#         H (torch.Tensor): Incidence matrix of shape (n, m), where H[i, j] = 1
...
#     # Hyperedges are treated as additional nodes indexed from n to n + m - 1
...
#     edge_index = torch.stack([row1, row2], dim=0)  # shape: (2, 2*num_edges)
#     return edge_index
...
def incidence_to_edge_index2(H: torch.Tensor) -> torch.Tensor:
    ...
    edge_nodes = edge_idx + n
    # print('node_idx.shape: ', node_idx.shape)
    # print('edge_nodes.shape: ', edge_nodes.shape)
