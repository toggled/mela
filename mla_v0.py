...
def meta_laplacian_attack(H, X, y, model, budget=20, epsilon=0.05, T=20, eta_H=1e-2, eta_X=1e-2, alpha=4.0, reinit_if_stuck=True):
    H = H.clone().detach()
    X = X.clone().detach()
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(args.dataset+'_'+args.model+'_'+str(args.ptb_rate)+'_loss_components.png')
    ...
    if class_acc_np.ndim == 2:
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig(args.dataset+'_'+args.model+'_'+str(args.ptb_rate)+'_class_acc.png')
...
if __name__ == '__main__':
    ...
    H_adv, X_adv = \
        meta_laplacian_attack(H, X, y, model, budget=perturbations, \
